...
function buildGallery($dir)
{
    $small =  $dir . '\small';
    $big =  $dir . '\big';
    $arr = array_slice(scandir($small), 2);
    $str = '';
    foreach ($arr as $Item) {
        if (!is_dir($Item)) {
            $str .= "<a rel='gallery' class='photo' href='{$big}/{$Item}'><img src='{$small}/{$Item}' width='150' height='100' /></a>";
        }
    }
    return $str;
}
...
{
    $str = '';
    $str .= "<img src='' width='150' height='100' /></a>";
    return "Hello";
}
